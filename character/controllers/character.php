...
class Character extends MX_Controller
{
	...
	public function getItem($id = false)
	{
		if($id != false)
		{
			...
			if($cache !== false)
			{
				...
				if($cache2 != false)
				{
					return "<a href='" . $this->template->page_url . "item/" . $this->realm . "/" . $id . "' rel='item=".$id."' data-realm='".$this->realm."'></a><img src='/wow-zamimg/static/images/icons/large/".$cache2.".jpg' />";
				}
				else
				{
					return "<a href='" . $this->template->page_url . "item/" . $this->realm . "/" . $id . "' rel='item=".$id."' data-realm='".$this->realm."'></a><img src='/wow-zamimg/static/images/icons/large/inv_misc_questionmark.jpg' />";
				}
			}
			else
			{
	...
	private function getInfo()
	{
		...
		{
			$character_stats = array('health' => lang("unknown", "character"));
		}
