		...
		<section id="store_content">
			...
			<section id="store_realms">
				...
				<section class="realm_items" id="realm_items_{$realmId}">
                    {if is_array($realm.items.groups)}
					...
					<section class="item_group" {if $minimize}style="display:none"{/if}>
						...
						<div class="store_item" id="item_{$item.id}">
							...
							<img class="item_icon" src="/wow-zamimg/static/images/icons/large/{$item.icon}.jpg" align="absmiddle" width="36px" {if $item.tooltip}data-realm="{$item.realm}" rel="item={$item.itemid}"{/if}>
